...
static void *policy_create(const char *unused_key, void *context)
{
    ...
    /*
     * Save level as policy level (may be downgraded by early fallback,
     * and compute fallback level if not specified per-site.
     */
					           ...
					           const char *fmt,...)
{
	...
	break;
    default:
	msg_panic("Invalid DANE security level: \"%s\"",
...
static void dane_init(SMTP_TLS_POLICY *tls, SMTP_ITERATOR *iter)
{
    ...
    /*
     * When TLSA lookups fail, as with dane-only, we fallback or defer the
     * message, the level will be set to either the fallback level or
     * "invalid", so we set to "dane-only", avoiding automatic downgrade to
     * may or encrypt as when records are missing or unusable with "dane".
     */
    if ((dane = tls_dane_resolve(iter->port, "tcp", iter->rr,
				 var_smtp_tls_force_tlsa)) == 0) {
	tls->level = TLS_LEV_DANE_ONLY;
	dane_incompat(tls, iter, NONDANE_DEST, "%s:%u: DANE TLSA lookup error",
